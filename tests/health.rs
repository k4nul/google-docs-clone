...
use backend::{
    ...
    storage::{
        ...
        JsondbSnapshotStore, KafiSnapshotStore, KagiSnapshotStore, KoitSnapshotStore,
        KopperdbSnapshotStore, KstoneSnapshotStore, KvSnapshotStore, LedgerKvSnapshotStore,
        LiteDbSnapshotStore, LmdbRsCoreSnapshotStore, LogKvSnapshotStore, LoroKvSnapshotStore,
...
fn test_config() -> Config {
    Config {
        ...
        snapshot_kagi_path: "./data/test-snapshots.kagi".to_owned(),
        ...
        snapshot_png_db_path: "./data/test-snapshots.png".to_owned(),
...
fn configure_json_mutex_db_snapshot_store(config: &mut Config, root: &std::path::Path) {
    ...
    config.snapshot_json_mutex_db_path = root
        .join("snapshots.json_mutex_db.json")
        .to_string_lossy()
        .into_owned();
}
...
fn app_state_uses_png_db_snapshot_store_from_config() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    configure_png_db_snapshot_store(&mut config, &snapshot_dir);

    let state = AppState::from_config(&config).expect("state should initialize with png_db store");

    let document = state
        .rooms()
        .create_document(Some("Persisted to png_db".to_owned()))
        .expect("document should be created");
    let room = state
        .rooms()
        .get(&document.id)
    ...
    let teardown = state
        .rooms()
        .persist_and_evict_if_idle(&document.id, &room)
        .expect("snapshot should persist to png_db on eviction");
    assert!(teardown.evicted);
    assert_eq!(teardown.remaining_sessions, 0);
    ...
    let reloaded_state =
        AppState::from_config(&config).expect("state should reload persisted png_db snapshot");
    let restored_room = reloaded_state
        .rooms()
        .get(&document.id)
        .expect("persisted room should hydrate on startup");
    ...
    assert_eq!(restored_room.document().id, document.id);
    assert!(snapshot_dir.join("snapshots.png").exists());
...
fn app_state_uses_dir_cache_snapshot_store_from_config() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    let snapshot_path = snapshot_dir.join("snapshots.dir_cache");
    configure_dir_cache_snapshot_store(&mut config, &snapshot_dir);

    let state =
        AppState::from_config(&config).expect("state should initialize with dir_cache store");

    let document = state
        .rooms()
        .create_document(Some("Persisted to dir_cache".to_owned()))
        .expect("document should be created");
    let room = state
        .rooms()
        .get(&document.id)
    ...
    let teardown = state
        .rooms()
        .persist_and_evict_if_idle(&document.id, &room)
        .expect("snapshot should persist to dir_cache on eviction");
    assert!(teardown.evicted);
    assert_eq!(teardown.remaining_sessions, 0);
    ...
    let reloaded_state =
        AppState::from_config(&config).expect("state should reload persisted dir_cache snapshot");
    let restored_room = reloaded_state
        .rooms()
        .get(&document.id)
        .expect("persisted room should hydrate on startup");
...
fn app_state_uses_json_db_rs_snapshot_store_from_config() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    let snapshot_path = snapshot_dir.join("snapshots.json_db_rs.json");
    configure_json_db_rs_snapshot_store(&mut config, &snapshot_dir);

    let state =
        AppState::from_config(&config).expect("state should initialize with json_db_rs store");

    let document = state
        .rooms()
        .create_document(Some("Persisted to json_db_rs".to_owned()))
        .expect("document should be created");
    let room = state
        .rooms()
        .get(&document.id)
    ...
    let teardown = state
        .rooms()
        .persist_and_evict_if_idle(&document.id, &room)
        .expect("snapshot should persist to json_db_rs on eviction");
    assert!(teardown.evicted);
    assert_eq!(teardown.remaining_sessions, 0);
...
fn app_state_uses_json_mutex_db_snapshot_store_from_config() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    let snapshot_path = snapshot_dir.join("snapshots.json_mutex_db.json");
    configure_json_mutex_db_snapshot_store(&mut config, &snapshot_dir);

    let state =
        AppState::from_config(&config).expect("state should initialize with json_mutex_db backend");

    let document = state
        .rooms()
        .create_document(Some("Persisted to json_mutex_db".to_owned()))
        .expect("document should be created");
    let room = state
        .rooms()
        .get(&document.id)
    ...
    let teardown = state
        .rooms()
        .persist_and_evict_if_idle(&document.id, &room)
        .expect("snapshot should persist to json_mutex_db on eviction");
    assert!(teardown.evicted);
    assert_eq!(teardown.remaining_sessions, 0);
    ...
    let reloaded_state = AppState::from_config(&config)
        .expect("state should reload persisted json_mutex_db snapshot");
    let restored_room = reloaded_state
        .rooms()
        .get(&document.id)
        .expect("persisted room should hydrate on startup");
...
fn app_state_uses_toiletdb_snapshot_store_from_config() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    let snapshot_path = snapshot_dir.join("snapshots.toiletdb.json");
    configure_toiletdb_snapshot_store(&mut config, &snapshot_dir);

    let state =
        AppState::from_config(&config).expect("state should initialize with toiletdb backend");

    let document = state
        .rooms()
        .create_document(Some("Persisted to toiletdb".to_owned()))
        .expect("document should be created");
    let room = state
        .rooms()
        .get(&document.id)
    ...
    let teardown = state
        .rooms()
        .persist_and_evict_if_idle(&document.id, &room)
        .expect("snapshot should persist to toiletdb on eviction");
    assert!(teardown.evicted);
    assert_eq!(teardown.remaining_sessions, 0);
    ...
    let reloaded_state =
        AppState::from_config(&config).expect("state should reload persisted toiletdb snapshot");
    let restored_room = reloaded_state
        .rooms()
        .get(&document.id)
        .expect("persisted room should hydrate on startup");
...
fn app_state_uses_marble_snapshot_store_from_config() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    let snapshot_path = snapshot_dir.join("snapshots.marble");
    configure_marble_snapshot_store(&mut config, &snapshot_dir);

    let state = AppState::from_config(&config).expect("state should initialize with marble store");

    let document = state
        .rooms()
        .create_document(Some("Persisted to marble".to_owned()))
        .expect("document should be created");
    let room = state
        .rooms()
        .get(&document.id)
    ...
    let teardown = state
        .rooms()
        .persist_and_evict_if_idle(&document.id, &room)
        .expect("snapshot should persist to marble on eviction");
    assert!(teardown.evicted);
    assert_eq!(teardown.remaining_sessions, 0);
    ...
    let reloaded_state =
        AppState::from_config(&config).expect("state should reload persisted marble snapshot");
    let restored_room = reloaded_state
        .rooms()
        .get(&document.id)
        .expect("persisted room should hydrate on startup");
    ...
    assert_eq!(restored_room.document().id, document.id);
    assert!(snapshot_path.join("heap").exists());
...
fn app_state_uses_kagi_snapshot_store_from_config() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    let snapshot_path = snapshot_dir.join("snapshots.kagi");
    configure_kagi_snapshot_store(&mut config, &snapshot_dir);

    let state = AppState::from_config(&config).expect("state should initialize with kagi store");

    let document = state
        .rooms()
        .create_document(Some("Persisted to kagi".to_owned()))
        .expect("document should be created");
    let room = state
        .rooms()
        .get(&document.id)
    ...
    let teardown = state
        .rooms()
        .persist_and_evict_if_idle(&document.id, &room)
        .expect("snapshot should persist to kagi on eviction");
    assert!(teardown.evicted);
    assert_eq!(teardown.remaining_sessions, 0);
    ...
    let reloaded_state =
        AppState::from_config(&config).expect("state should reload persisted kagi snapshot");
    let restored_room = reloaded_state
        .rooms()
        .get(&document.id)
        .expect("persisted room should hydrate on startup");
...
fn app_state_uses_deeb_snapshot_store_from_config() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    let snapshot_path = snapshot_dir.join("snapshots.deeb.json");
    configure_deeb_snapshot_store(&mut config, &snapshot_dir);

    let state = AppState::from_config(&config).expect("state should initialize with deeb store");

    let document = state
        .rooms()
        .create_document(Some("Persisted to deeb".to_owned()))
        .expect("document should be created");
    let room = state
        .rooms()
        .get(&document.id)
    ...
    let teardown = state
        .rooms()
        .persist_and_evict_if_idle(&document.id, &room)
        .expect("snapshot should persist to deeb on eviction");
    assert!(teardown.evicted);
    assert_eq!(teardown.remaining_sessions, 0);
    ...
    let reloaded_state =
        AppState::from_config(&config).expect("state should reload persisted deeb snapshot");
    let restored_room = reloaded_state
        .rooms()
        .get(&document.id)
        .expect("persisted room should hydrate on startup");
...
fn app_state_uses_saturn_snapshot_store_from_config() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    let snapshot_path = snapshot_dir.join("snapshots.saturn");
    configure_saturn_snapshot_store(&mut config, &snapshot_dir);

    let state = AppState::from_config(&config).expect("state should initialize with saturn store");

    let document = state
        .rooms()
        .create_document(Some("Persisted to saturn".to_owned()))
        .expect("document should be created");
    let room = state
        .rooms()
        .get(&document.id)
    ...
    let teardown = state
        .rooms()
        .persist_and_evict_if_idle(&document.id, &room)
        .expect("snapshot should persist to saturn on eviction");
    assert!(teardown.evicted);
    assert_eq!(teardown.remaining_sessions, 0);
    ...
    let reloaded_state =
        AppState::from_config(&config).expect("state should reload persisted saturn snapshot");
    let restored_room = reloaded_state
        .rooms()
        .get(&document.id)
        .expect("persisted room should hydrate on startup");
...
#[test]
fn file_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    let snapshot_dir = temp_snapshot_dir("file-store-migration");
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Disk".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);

    let store = FileSnapshotStore::new(&snapshot_dir).expect("file store should initialize");
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to file store");
    drop(store);

    let reopened_store = FileSnapshotStore::new(&snapshot_dir).expect("file store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from file store after reopen");
    assert!(
        reopened_store
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect file store deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Disk".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        FileSnapshotStore::new(&snapshot_dir).expect("file store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    drop(replacement_store);

    let final_store = FileSnapshotStore::new(&snapshot_dir).expect("file store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement file snapshot"),
...
#[test]
fn sqlite_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    let snapshot_dir = temp_snapshot_dir("sqlite-store-migration");
    let snapshot_path = snapshot_dir.join("snapshots.sqlite3");
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Sqlite".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);

    let store = SqliteSnapshotStore::new(&snapshot_path).expect("sqlite store should initialize");
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to sqlite");
    drop(store);

    let reopened_store =
        SqliteSnapshotStore::new(&snapshot_path).expect("sqlite snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from sqlite after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect sqlite deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Sqlite".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement sqlite snapshot"),
        vec![replacement_document.clone()]
    );
...
fn jammdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Jammdb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to jammdb");
    drop(store);

    let reopened_store =
        JammdbSnapshotStore::new(&snapshot_path).expect("jammdb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from jammdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect jammdb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Jammdb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement jammdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn janql_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original JanQL".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to janql");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from janql after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect janql deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement JanQL".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        JanqlSnapshotStore::new(&snapshot_path).expect("janql snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement janql snapshot"),
        vec![replacement_document.clone()]
    );
...
fn jasondb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original JasonDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to jasondb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from jasondb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement JasonDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement jasondb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn jasonisnthappy_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original JasonIsntHappy".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to jasonisnthappy");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from jasonisnthappy after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement JasonIsntHappy".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = JasonisnthappySnapshotStore::new(&snapshot_path)
        .expect("jasonisnthappy snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = JasonisnthappySnapshotStore::new(&snapshot_path)
        .expect("jasonisnthappy snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement jasonisnthappy snapshot"),
        vec![replacement_document.clone()]
    );
...
fn datastack_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original DataStack".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to datastack");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from datastack after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement DataStack".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = DatastackSnapshotStore::new(&snapshot_path)
        .expect("datastack snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = DatastackSnapshotStore::new(&snapshot_path)
        .expect("datastack snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement datastack snapshot"),
        vec![replacement_document.clone()]
    );
...
fn mace_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Mace".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to mace");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from mace after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect mace deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Mace".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        MaceSnapshotStore::new(&snapshot_path).expect("mace snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement mace snapshot"),
        vec![replacement_document.clone()]
    );
...
fn heed_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Heed".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to heed");
    drop(store);

    let reopened_store =
        HeedSnapshotStore::new(&snapshot_path).expect("heed snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from heed after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect heed deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Heed".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        HeedSnapshotStore::new(&snapshot_path).expect("heed snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement heed snapshot"),
        vec![replacement_document.clone()]
    );
...
fn hightower_kv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original Hightower KV".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to hightower_kv");
    drop(store);

    let reopened_store = HightowerKvSnapshotStore::new(&snapshot_path)
        .expect("hightower_kv snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from hightower_kv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect hightower_kv deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Hightower KV".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = HightowerKvSnapshotStore::new(&snapshot_path)
        .expect("hightower_kv snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = HightowerKvSnapshotStore::new(&snapshot_path)
        .expect("hightower_kv snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement hightower_kv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn persy_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Persy".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to persy");
    drop(store);

    let reopened_store =
        PersySnapshotStore::new(&snapshot_path).expect("persy snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from persy after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect persy deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Persy".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        PersySnapshotStore::new(&snapshot_path).expect("persy snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement persy snapshot"),
        vec![replacement_document.clone()]
    );
...
fn redb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Redb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to redb");
    drop(store);

    let reopened_store =
        RedbSnapshotStore::new(&snapshot_path).expect("redb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from redb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect redb deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Redb".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        RedbSnapshotStore::new(&snapshot_path).expect("redb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement redb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn sled_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Sled".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to sled");
    drop(store);

    let reopened_store =
        SledSnapshotStore::new(&snapshot_path).expect("sled snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from sled after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect sled deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Sled".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        SledSnapshotStore::new(&snapshot_path).expect("sled snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement sled snapshot"),
        vec![replacement_document.clone()]
    );
...
fn pickledb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original PickleDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to pickledb");
    drop(store);

    let reopened_store =
        PickleDbSnapshotStore::new(&snapshot_path).expect("pickledb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from pickledb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect pickledb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement PickleDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement pickledb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn microkv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original MicroKV".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to microkv");
    drop(store);

    let reopened_store =
        MicroKvSnapshotStore::new(&snapshot_path).expect("microkv snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from microkv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect microkv deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement MicroKV".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement microkv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn yedb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Yedb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to yedb");
    drop(store);

    let reopened_store =
        YedbSnapshotStore::new(&snapshot_path).expect("yedb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from yedb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect yedb deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Yedb".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        YedbSnapshotStore::new(&snapshot_path).expect("yedb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement yedb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn btree_store_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original BtreeStore".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to btree_store");
    drop(store);

    let reopened_store = BtreeStoreSnapshotStore::new(&snapshot_path)
        .expect("btree_store snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from btree_store after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect btree_store deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement BtreeStore".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = BtreeStoreSnapshotStore::new(&snapshot_path)
        .expect("btree_store snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = BtreeStoreSnapshotStore::new(&snapshot_path)
        .expect("btree_store snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement btree_store snapshot"),
        vec![replacement_document.clone()]
    );
...
fn cacache_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Cacache".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to cacache");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from cacache after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Cacache".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement cacache snapshot"),
        vec![replacement_document.clone()]
    );
...
fn siamesedb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original Siamesedb".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to siamesedb");
    drop(store);

    let reopened_store = SiamesedbSnapshotStore::new(&snapshot_path)
        .expect("siamesedb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from siamesedb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect siamesedb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Siamesedb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = SiamesedbSnapshotStore::new(&snapshot_path)
        .expect("siamesedb snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = SiamesedbSnapshotStore::new(&snapshot_path)
        .expect("siamesedb snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement siamesedb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn readb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Readb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to readb");
    drop(store);

    let reopened_store =
        ReadbSnapshotStore::new(&snapshot_path).expect("readb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from readb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect readb deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Readb".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        ReadbSnapshotStore::new(&snapshot_path).expect("readb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement readb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn rustlite_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Rustlite".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to rustlite");
    drop(store);

    let reopened_store =
        RustliteSnapshotStore::new(&snapshot_path).expect("rustlite snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from rustlite after reopen");
    drop(reopened_store);
    ...
    assert!(
        deleted_store
            .list_documents()
            .expect("document catalog should reflect rustlite deletion")
            .is_empty()
    );
    drop(deleted_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Rustlite".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement rustlite snapshot"),
        vec![replacement_document.clone()]
    );
...
fn canopydb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Canopydb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to canopydb");
    drop(store);

    let reopened_store =
        CanopydbSnapshotStore::new(&snapshot_path).expect("canopydb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from canopydb after reopen");
    drop(reopened_store);
    ...
    assert!(
        deleted_store
            .list_documents()
            .expect("document catalog should reflect canopydb deletion")
            .is_empty()
    );
    drop(deleted_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Canopydb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement canopydb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn structsy_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Structsy".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to structsy");
    drop(store);

    let reopened_store =
        StructsySnapshotStore::new(&snapshot_path).expect("structsy snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from structsy after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect structsy deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Structsy".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement structsy snapshot"),
        vec![replacement_document.clone()]
    );
...
fn abyssiniandb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original Abyssiniandb".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to abyssiniandb");
    drop(store);

    let reopened_store = AbyssiniandbSnapshotStore::new(&snapshot_path)
        .expect("abyssiniandb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from abyssiniandb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect abyssiniandb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Abyssiniandb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = AbyssiniandbSnapshotStore::new(&snapshot_path)
        .expect("abyssiniandb snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = AbyssiniandbSnapshotStore::new(&snapshot_path)
        .expect("abyssiniandb snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement abyssiniandb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn ckydb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Ckydb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to ckydb");
    drop(store);

    let reopened_store =
        CkydbSnapshotStore::new(&snapshot_path).expect("ckydb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from ckydb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect ckydb deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Ckydb".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        CkydbSnapshotStore::new(&snapshot_path).expect("ckydb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement ckydb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn crepedb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original CrepeDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to crepedb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from crepedb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect crepedb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement CrepeDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement crepedb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn crystal_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Crystal".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to crystal");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from crystal after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Crystal".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement crystal snapshot"),
        vec![replacement_document.clone()]
    );
...
fn assystem_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Assystem".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to assystem");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from assystem after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Assystem".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement assystem snapshot"),
        vec![replacement_document.clone()]
    );
...
fn colon_db_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Colon DB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to colon_db");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from colon_db after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Colon DB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement colon_db snapshot"),
        vec![replacement_document.clone()]
    );
...
fn rskey_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Rskey".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to rskey");
    drop(store);

    let reopened_store =
        RskeySnapshotStore::new(&snapshot_path).expect("rskey snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from rskey after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect rskey deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Rskey".to_owned()));
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement rskey snapshot"),
        vec![replacement_document.clone()]
    );
...
fn scdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Scdb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to scdb");
    drop(store);

    let reopened_store =
        ScdbSnapshotStore::new(&snapshot_path).expect("scdb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from scdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect scdb deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Scdb".to_owned()));
    let replacement_snapshot = DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6]);
    let replacement_store =
        ScdbSnapshotStore::new(&snapshot_path).expect("scdb snapshot store should reopen");
    replacement_store
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement scdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn skv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Skv".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to skv");
    drop(store);

    let reopened_store =
        SkvSnapshotStore::new(&snapshot_path).expect("skv snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from skv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect skv deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Skv".to_owned()));
    let replacement_snapshot = DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6]);
    let replacement_store =
        SkvSnapshotStore::new(&snapshot_path).expect("skv snapshot store should reopen");
    replacement_store
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement skv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn surrealkv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original SurrealKV".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to surrealkv");
    drop(store);

    let reopened_store = SurrealkvSnapshotStore::new(&snapshot_path)
        .expect("surrealkv snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from surrealkv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect surrealkv deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement SurrealKV".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = SurrealkvSnapshotStore::new(&snapshot_path)
        .expect("surrealkv snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = SurrealkvSnapshotStore::new(&snapshot_path)
        .expect("surrealkv snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement surrealkv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn thunderdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original thunderdb".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to thunderdb");
    drop(store);

    let reopened_store = ThunderdbSnapshotStore::new(&snapshot_path)
        .expect("thunderdb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from thunderdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect thunderdb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement thunderdb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = ThunderdbSnapshotStore::new(&snapshot_path)
        .expect("thunderdb snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = ThunderdbSnapshotStore::new(&snapshot_path)
        .expect("thunderdb snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement thunderdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn thetadb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original thetadb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to thetadb");
    drop(store);

    let reopened_store =
        ThetadbSnapshotStore::new(&snapshot_path).expect("thetadb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from thetadb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect thetadb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement thetadb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement thetadb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn tinybase_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Tinybase".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to tinybase");
    drop(store);

    let reopened_store =
        TinybaseSnapshotStore::new(&snapshot_path).expect("tinybase snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from tinybase after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect tinybase deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Tinybase".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement tinybase snapshot"),
        vec![replacement_document.clone()]
    );
...
fn tinydb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Tinydb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to tinydb");
    drop(store);

    let reopened_store =
        TinydbSnapshotStore::new(&snapshot_path).expect("tinydb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from tinydb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect tinydb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Tinydb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement tinydb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn dblite_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Dblite".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to dblite");
    drop(store);

    let reopened_store =
        DbliteSnapshotStore::new(&snapshot_path).expect("dblite snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from dblite after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect dblite deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Dblite".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement dblite snapshot"),
        vec![replacement_document.clone()]
    );
...
fn aeternusdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original AeternusDB".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to aeternusdb");
    drop(store);

    let reopened_store = AeternusdbSnapshotStore::new(&snapshot_path)
        .expect("aeternusdb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from aeternusdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement AeternusDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = AeternusdbSnapshotStore::new(&snapshot_path)
        .expect("aeternusdb snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = AeternusdbSnapshotStore::new(&snapshot_path)
        .expect("aeternusdb snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement aeternusdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn dbless_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original dbless".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to dbless");
    drop(store);

    let reopened_store =
        DblessSnapshotStore::new(&snapshot_path).expect("dbless snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from dbless after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement dbless".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement dbless snapshot"),
        vec![replacement_document.clone()]
    );
...
fn sanakirja_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original Sanakirja".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to sanakirja");
    drop(store);

    let reopened_store = SanakirjaSnapshotStore::new(&snapshot_path)
        .expect("sanakirja snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from sanakirja after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect sanakirja deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Sanakirja".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = SanakirjaSnapshotStore::new(&snapshot_path)
        .expect("sanakirja snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = SanakirjaSnapshotStore::new(&snapshot_path)
        .expect("sanakirja snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement sanakirja snapshot"),
        vec![replacement_document.clone()]
    );
...
fn flash_kv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original flash_kv".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to flash_kv");
    drop(store);

    let reopened_store =
        FlashKvSnapshotStore::new(&snapshot_path).expect("flash_kv snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from flash_kv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect flash_kv deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement flash_kv".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement flash_kv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn grebedb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original grebedb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to grebedb");
    drop(store);

    let reopened_store =
        GrebedbSnapshotStore::new(&snapshot_path).expect("grebedb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from grebedb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect nikidb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement grebedb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement grebedb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn grumpydb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original grumpydb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to grumpydb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from grumpydb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement grumpydb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement grumpydb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn highlandcows_isam_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original Highlandcows ISAM".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to highlandcows-isam");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from highlandcows-isam after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Highlandcows ISAM".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = HighlandcowsIsamSnapshotStore::new(&snapshot_path)
        .expect("highlandcows-isam snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = HighlandcowsIsamSnapshotStore::new(&snapshot_path)
        .expect("highlandcows-isam snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement highlandcows-isam snapshot"),
        vec![replacement_document.clone()]
    );
...
fn simple_db_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original SimpleDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to simple_db");
    drop(store);

    let reopened_store =
        SimpleDbSnapshotStore::new(&snapshot_path).expect("simple_db snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from simple_db after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect simple_db deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement SimpleDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement simple_db snapshot"),
        vec![replacement_document.clone()]
    );
...
fn snaildb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original SnailDb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to snaildb");
    drop(store);

    let reopened_store =
        SnaildbSnapshotStore::new(&snapshot_path).expect("snaildb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from snaildb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect snaildb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement SnailDb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement snaildb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn docdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original DocDb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to docdb");
    drop(store);

    let reopened_store =
        DocDbSnapshotStore::new(&snapshot_path).expect("docdb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from docdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect docdb deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement DocDb".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        DocDbSnapshotStore::new(&snapshot_path).expect("docdb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement docdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn shorterdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original ShorterDB".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to shorterdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from shorterdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect shorterdb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement ShorterDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = ShorterDbSnapshotStore::new(&snapshot_path)
        .expect("shorterdb snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = ShorterDbSnapshotStore::new(&snapshot_path)
        .expect("shorterdb snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement shorterdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn tinykv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original TinyKV".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to tinykv");
    drop(store);

    let reopened_store =
        TinykvSnapshotStore::new(&snapshot_path).expect("tinykv snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from tinykv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect tinykv deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement TinyKV".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement tinykv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn saberdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original SaberDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to saberdb");
    drop(store);

    let reopened_store =
        SaberdbSnapshotStore::new(&snapshot_path).expect("saberdb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from saberdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect saberdb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement SaberDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement saberdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn smolldb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original SmollDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to smolldb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from smolldb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement SmollDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement smolldb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn kstone_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Kstone".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to kstone");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from kstone after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Kstone".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement kstone snapshot"),
        vec![replacement_document.clone()]
    );
...
fn ghaladb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original GhalaDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to ghaladb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from ghaladb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement GhalaDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement ghaladb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn roughdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original RoughDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to roughdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from roughdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement RoughDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement roughdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn raindb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original RainDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to raindb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from raindb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement RainDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement raindb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn infusedb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original InfuseDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to infusedb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from infusedb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement InfuseDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement infusedb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn kafi_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Kafi".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to kafi");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from kafi after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Kafi".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        KafiSnapshotStore::new(&snapshot_path).expect("kafi snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement kafi snapshot"),
        vec![replacement_document.clone()]
    );
...
fn tinkv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Tinkv".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to tinkv");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from tinkv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Tinkv".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        TinkvSnapshotStore::new(&snapshot_path).expect("tinkv snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement tinkv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn ledger_kv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original LedgerKV".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to ledger_kv");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from ledger_kv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement LedgerKV".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement ledger_kv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn joydb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Joydb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to joydb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from joydb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Joydb".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        JoydbSnapshotStore::new(&snapshot_path).expect("joydb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement joydb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn png_db_snapshot_store_round_trips_document_catalog() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    let snapshot_path = snapshot_dir.join("snapshots.png");
    let store =
        PngDbSnapshotStore::new(&snapshot_path).expect("png_db snapshot store should initialize");
    let document =
        backend::models::document::Document::new(Uuid::new_v4(), Some("PNG DB".to_owned()));
    let snapshot = DocumentSnapshot::new(document.clone(), vec![1, 2, 3]);

    store
        .save_snapshot(snapshot)
        .expect("snapshot should save to png_db");

    let listed_documents = store
        .list_documents()
        .expect("document catalog should load from png_db");
    let loaded_snapshot = store
        .load_snapshot(&document.id)
        .expect("snapshot should load from png_db")
        .expect("snapshot should exist");
    ...
    let reopened_store =
        PngDbSnapshotStore::new(&snapshot_path).expect("png_db snapshot store should reopen");
    assert_eq!(
        reopened_store
            .list_documents()
            .expect("document catalog should reload from png_db"),
        vec![document.clone()]
    );
    assert!(
        reopened_store
            .load_snapshot(&document.id)
            .expect("snapshot should reload from png_db")
            .is_some()
    );

    reopened_store
        .delete_snapshot(&document.id)
        .expect("snapshot should delete from png_db");
    assert!(
        reopened_store
            .load_snapshot(&document.id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect png_db deletion")
            .is_empty()
    );
...
fn png_db_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original PNG DB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to png_db");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from png_db after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement PNG DB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement png_db snapshot"),
        vec![replacement_document.clone()]
    );
...
fn lsm_tree_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original LsmTree".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to lsm_tree");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from lsm_tree after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement LsmTree".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement lsm_tree snapshot"),
        vec![replacement_document.clone()]
    );
...
fn bitcask_engine_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original Bitcask Engine".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to bitcask-engine");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from bitcask-engine after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Bitcask Engine".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = BitcaskEngineSnapshotStore::new(&snapshot_path)
        .expect("bitcask-engine snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = BitcaskEngineSnapshotStore::new(&snapshot_path)
        .expect("bitcask-engine snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement bitcask-engine snapshot"),
        vec![replacement_document.clone()]
    );
...
fn feoxdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original FeOxDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to feoxdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from feoxdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect feoxdb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement FeOxDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement feoxdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn agdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original agdb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to agdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from agdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect agdb deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement agdb".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        AgdbSnapshotStore::new(&snapshot_path).expect("agdb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement agdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn amandine_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Amandine".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to amandine");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from amandine after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect amandine deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Amandine".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement amandine snapshot"),
        vec![replacement_document.clone()]
    );
...
fn armdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original ArmDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to armdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from armdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement ArmDB".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        ArmdbSnapshotStore::new(&snapshot_path).expect("armdb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement armdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn db_rs_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original db_rs".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to db_rs");
    drop(store);

    let reopened_store =
        DbRsSnapshotStore::new(&snapshot_path).expect("db_rs snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from db_rs after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect db_rs deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement db_rs".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        DbRsSnapshotStore::new(&snapshot_path).expect("db_rs snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement db_rs snapshot"),
        vec![replacement_document.clone()]
    );
...
fn dharmadb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original DharmaDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to dharmadb");
    drop(store);

    let reopened_store =
        DharmadbSnapshotStore::new(&snapshot_path).expect("dharmadb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from dharmadb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect dharmadb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement DharmaDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement dharmadb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn dir_cache_snapshot_store_round_trips_document_catalog() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    let snapshot_path = snapshot_dir.join("snapshots.dir_cache");
    let store = DirCacheSnapshotStore::new(&snapshot_path)
        .expect("dir_cache snapshot store should initialize");
    let document =
        backend::models::document::Document::new(Uuid::new_v4(), Some("DirCache".to_owned()));
    let snapshot = DocumentSnapshot::new(document.clone(), vec![1, 2, 3]);

    store
        .save_snapshot(snapshot)
        .expect("snapshot should save to dir_cache");

    let listed_documents = store
        .list_documents()
        .expect("document catalog should load from dir_cache");
    let loaded_snapshot = store
        .load_snapshot(&document.id)
        .expect("snapshot should load from dir_cache")
        .expect("snapshot should exist");
    ...
    let reopened =
        DirCacheSnapshotStore::new(&snapshot_path).expect("dir_cache snapshot store should reopen");
    let reopened_documents = reopened
        .list_documents()
        .expect("document catalog should reload from dir_cache");
    let reopened_snapshot = reopened
        .load_snapshot(&document.id)
        .expect("snapshot should reload from dir_cache")
        .expect("snapshot should exist after reopen");
...
fn dir_cache_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original DirCache".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to dir_cache");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from dir_cache after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect dir_cache deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement DirCache".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement dir_cache snapshot"),
        vec![replacement_document.clone()]
    );
...
fn jsondb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original JsonDb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to jsondb");
    drop(store);

    let reopened_store =
        JsondbSnapshotStore::new(&snapshot_path).expect("jsondb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from jsondb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement JsonDb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement jsondb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn kopperdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original KopperDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to kopperdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from kopperdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement KopperDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement kopperdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn rcask_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original rcask".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to rcask");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from rcask after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement rcask".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        RcaskSnapshotStore::new(&snapshot_path).expect("rcask snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement rcask snapshot"),
        vec![replacement_document.clone()]
    );
...
fn json_store_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original JsonStore".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to json_store");
    drop(store);

    let reopened_store = JsonStoreSnapshotStore::new(&snapshot_path)
        .expect("json_store snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from json_store after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement JsonStore".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = JsonStoreSnapshotStore::new(&snapshot_path)
        .expect("json_store snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = JsonStoreSnapshotStore::new(&snapshot_path)
        .expect("json_store snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement json_store snapshot"),
        vec![replacement_document.clone()]
    );
...
fn json_db_rs_snapshot_store_round_trips_document_catalog() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    let snapshot_path = snapshot_dir.join("snapshots.json_db_rs.json");
    let store = JsonDbRsSnapshotStore::new(&snapshot_path)
        .expect("json_db_rs snapshot store should initialize");
    let document =
        backend::models::document::Document::new(Uuid::new_v4(), Some("JsonDbRs".to_owned()));
    let snapshot = DocumentSnapshot::new(document.clone(), vec![1, 2, 3]);

    store
        .save_snapshot(snapshot)
        .expect("snapshot should save to json_db_rs");

    let listed_documents = store
        .list_documents()
        .expect("document catalog should load from json_db_rs");
    let loaded_snapshot = store
        .load_snapshot(&document.id)
        .expect("snapshot should load from json_db_rs")
        .expect("snapshot should exist");
    ...
    let reopened_store = JsonDbRsSnapshotStore::new(&snapshot_path)
        .expect("json_db_rs snapshot store should reopen");
    assert_eq!(
        reopened_store
            .list_documents()
            .expect("document catalog should reload from json_db_rs"),
        vec![document]
    );
...
fn json_db_rs_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original JsonDbRs".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to json_db_rs");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from json_db_rs after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement JsonDbRs".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = JsonDbRsSnapshotStore::new(&snapshot_path)
        .expect("json_db_rs snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = JsonDbRsSnapshotStore::new(&snapshot_path)
        .expect("json_db_rs snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement json_db_rs snapshot"),
        vec![replacement_document.clone()]
    );
...
fn cdb64_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Cdb64".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to cdb64");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from cdb64 after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Cdb64".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        Cdb64SnapshotStore::new(&snapshot_path).expect("cdb64 snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement cdb64 snapshot"),
        vec![replacement_document.clone()]
    );
...
fn json_mutex_db_snapshot_store_round_trips_document_catalog() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    let snapshot_path = snapshot_dir.join("snapshots.json_mutex_db.json");
    let store = JsonMutexDbSnapshotStore::new(&snapshot_path)
        .expect("json_mutex_db snapshot store should initialize");
    let document =
        backend::models::document::Document::new(Uuid::new_v4(), Some("JsonMutexDB".to_owned()));
    let snapshot = DocumentSnapshot::new(document.clone(), vec![1, 2, 3]);

    store
        .save_snapshot(snapshot)
        .expect("snapshot should save to json_mutex_db");

    let listed_documents = store
        .list_documents()
        .expect("document catalog should load from json_mutex_db");
    let loaded_snapshot = store
        .load_snapshot(&document.id)
        .expect("snapshot should load from json_mutex_db")
        .expect("snapshot should exist");
    ...
    let reopened = JsonMutexDbSnapshotStore::new(&snapshot_path)
        .expect("json_mutex_db snapshot store should reopen");
    let reopened_documents = reopened
        .list_documents()
        .expect("document catalog should reload from json_mutex_db");
    let reopened_snapshot = reopened
        .load_snapshot(&document.id)
        .expect("snapshot should reload from json_mutex_db")
        .expect("snapshot should exist after reopen");
...
fn json_mutex_db_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original JsonMutexDB".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to json_mutex_db");
    drop(store);

    let reopened_store = JsonMutexDbSnapshotStore::new(&snapshot_path)
        .expect("json_mutex_db snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from json_mutex_db after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement JsonMutexDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = JsonMutexDbSnapshotStore::new(&snapshot_path)
        .expect("json_mutex_db snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = JsonMutexDbSnapshotStore::new(&snapshot_path)
        .expect("json_mutex_db snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement json_mutex_db snapshot"),
        vec![replacement_document.clone()]
    );
...
fn toiletdb_snapshot_store_round_trips_document_catalog() {
    ...
    fs::create_dir_all(&snapshot_dir).expect("test snapshot directory should be created");
    let snapshot_path = snapshot_dir.join("snapshots.toiletdb.json");
    let store = ToiletdbSnapshotStore::new(&snapshot_path)
        .expect("toiletdb snapshot store should initialize");
    let document =
        backend::models::document::Document::new(Uuid::new_v4(), Some("ToiletDB".to_owned()));
    let snapshot = DocumentSnapshot::new(document.clone(), vec![1, 2, 3]);

    store
        .save_snapshot(snapshot)
        .expect("snapshot should save to toiletdb");

    let listed_documents = store
        .list_documents()
        .expect("document catalog should load from toiletdb");
    let loaded_snapshot = store
        .load_snapshot(&document.id)
        .expect("snapshot should load from toiletdb")
        .expect("snapshot should exist");
    ...
    let reopened =
        ToiletdbSnapshotStore::new(&snapshot_path).expect("toiletdb snapshot store should reopen");
    let reopened_documents = reopened
        .list_documents()
        .expect("document catalog should reload from toiletdb");
    let reopened_snapshot = reopened
        .load_snapshot(&document.id)
        .expect("snapshot should reload from toiletdb")
        .expect("snapshot should exist after reopen");
    ...
    reopened
        .delete_snapshot(&reopened_documents[0].id)
        .expect("snapshot should delete from toiletdb");
    assert!(
        reopened
            .list_documents()
            .expect("document catalog should reflect toiletdb deletion")
            .is_empty()
    );
...
fn toiletdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original ToiletDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to toiletdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from toiletdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement ToiletDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement toiletdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn koit_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Koit".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to koit");
    drop(store);

    let reopened_store =
        KoitSnapshotStore::new(&snapshot_path).expect("koit snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from koit after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Koit".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        KoitSnapshotStore::new(&snapshot_path).expect("koit snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement koit snapshot"),
        vec![replacement_document.clone()]
    );
...
fn lite_db_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original LiteDb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to lite_db");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from lite_db after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement LiteDb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement lite_db snapshot"),
        vec![replacement_document.clone()]
    );
...
fn data_pile_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original DataPile".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to data_pile");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from data_pile after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement DataPile".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement data_pile snapshot"),
        vec![replacement_document.clone()]
    );
...
fn log_kv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original LogKV".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to log_kv");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from log_kv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement LogKV".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        LogKvSnapshotStore::new(&snapshot_path).expect("log_kv snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement log_kv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn append_kv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original AppendKV".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to append_kv");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from append_kv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement AppendKV".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement append_kv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn append_log_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original AppendLog".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to append_log");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from append_log after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement AppendLog".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = AppendLogSnapshotStore::new(&snapshot_path)
        .expect("append_log snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = AppendLogSnapshotStore::new(&snapshot_path)
        .expect("append_log snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement append_log snapshot"),
        vec![replacement_document.clone()]
    );
...
fn mhdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original MHdb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to mhdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from mhdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement MHdb".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        MhdbSnapshotStore::new(&snapshot_path).expect("mhdb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement mhdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn marble_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Marble".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);

    let store =
        MarbleSnapshotStore::new(&snapshot_path).expect("marble snapshot store should initialize");
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to marble");
    drop(store);

    let reopened_store =
        MarbleSnapshotStore::new(&snapshot_path).expect("marble snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from marble after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect marble deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Marble".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement marble snapshot"),
        vec![replacement_document.clone()]
    );
...
fn marble_snapshot_store_round_trips_document_catalog() {
    ...
    let store =
        MarbleSnapshotStore::new(&snapshot_path).expect("marble snapshot store should initialize");
    let document =
        backend::models::document::Document::new(Uuid::new_v4(), Some("Marble".to_owned()));
    let snapshot = DocumentSnapshot::new(document.clone(), vec![1, 2, 3]);

    store
        .save_snapshot(snapshot)
        .expect("snapshot should save to marble");

    let listed_documents = store
        .list_documents()
        .expect("document catalog should load from marble");
    let loaded_snapshot = store
        .load_snapshot(&document.id)
        .expect("snapshot should load from marble")
        .expect("snapshot should exist");
    ...
    assert_eq!(
        reopened_store
            .list_documents()
            .expect("document catalog should reload from marble"),
        vec![document.clone()]
    );
    assert!(
        reopened_store
            .load_snapshot(&document.id)
            .expect("snapshot should reload from marble")
            .is_some()
    );

    reopened_store
        .delete_snapshot(&document.id)
        .expect("snapshot should delete from marble");
    assert!(
        reopened_store
            .load_snapshot(&document.id)
            .expect("deleted snapshot lookup should succeed")
...
fn loro_kv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original LoroKV".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to loro_kv");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from loro_kv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement LoroKV".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement loro_kv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn luckdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original LuckDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to luckdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from luckdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement LuckDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement luckdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn ipjdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Ipjdb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to ipjdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from ipjdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Ipjdb".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        IpjdbSnapshotStore::new(&snapshot_path).expect("ipjdb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement ipjdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn kagi_snapshot_store_round_trips_document_catalog() {
    ...
    let snapshot_path = snapshot_dir.join("snapshots.kagi");
    let store =
        KagiSnapshotStore::new(&snapshot_path).expect("kagi snapshot store should initialize");
    let document =
        backend::models::document::Document::new(Uuid::new_v4(), Some("kagi".to_owned()));
    let snapshot = DocumentSnapshot::new(document.clone(), vec![1, 2, 3]);

    store
        .save_snapshot(snapshot)
        .expect("snapshot should save to kagi");

    let listed_documents = store
        .list_documents()
        .expect("document catalog should load from kagi");
    let loaded_snapshot = store
        .load_snapshot(&document.id)
        .expect("snapshot should load from kagi")
        .expect("snapshot should exist");
    ...
    let reopened_store =
        KagiSnapshotStore::new(&snapshot_path).expect("kagi snapshot store should reopen");
    assert_eq!(
        reopened_store
            .list_documents()
            .expect("document catalog should reload from kagi"),
        vec![document.clone()]
    );
    assert!(
        reopened_store
            .load_snapshot(&document.id)
            .expect("snapshot should reload from kagi")
            .is_some()
    );

    reopened_store
        .delete_snapshot(&document.id)
        .expect("snapshot should delete from kagi");
    assert!(
        reopened_store
            .load_snapshot(&document.id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect kagi deletion")
            .is_empty()
    );
...
fn kagi_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Kagi".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to kagi");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from kagi after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Kagi".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        KagiSnapshotStore::new(&snapshot_path).expect("kagi snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement kagi snapshot"),
        vec![replacement_document.clone()]
    );
...
fn deeb_snapshot_store_round_trips_document_catalog() {
    ...
    let snapshot_path = snapshot_dir.join("snapshots.deeb.json");
    let store =
        DeebSnapshotStore::new(&snapshot_path).expect("deeb snapshot store should initialize");
    let document =
        backend::models::document::Document::new(Uuid::new_v4(), Some("Deeb".to_owned()));
    let snapshot = DocumentSnapshot::new(document.clone(), vec![1, 2, 3]);

    store
        .save_snapshot(snapshot)
        .expect("snapshot should save to deeb");

    let listed_documents = store
        .list_documents()
        .expect("document catalog should load from deeb");
    let loaded_snapshot = store
        .load_snapshot(&document.id)
        .expect("snapshot should load from deeb")
        .expect("snapshot should exist");
    ...
    let reopened_store =
        DeebSnapshotStore::new(&snapshot_path).expect("deeb snapshot store should reopen");
    assert_eq!(
        reopened_store
            .list_documents()
            .expect("document catalog should reload from deeb"),
        vec![document.clone()]
    );
    assert!(
        reopened_store
            .load_snapshot(&document.id)
            .expect("snapshot should reload from deeb")
            .is_some()
    );

    reopened_store
        .delete_snapshot(&document.id)
        .expect("snapshot should delete from deeb");
    assert!(
        reopened_store
            .load_snapshot(&document.id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect deeb deletion")
            .is_empty()
    );
...
fn deeb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Deeb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to deeb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from deeb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Deeb".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        DeebSnapshotStore::new(&snapshot_path).expect("deeb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement deeb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn rubin_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Rubin".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to rubin");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from rubin after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Rubin".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        RubinSnapshotStore::new(&snapshot_path).expect("rubin snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement rubin snapshot"),
        vec![replacement_document.clone()]
    );
...
fn lsm_storage_engine_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original LSM storage engine".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to lsm_storage_engine");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from lsm_storage_engine after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect lsm_storage_engine deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement LSM storage engine".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = LsmStorageEngineSnapshotStore::new(&snapshot_path)
        .expect("lsm_storage_engine snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = LsmStorageEngineSnapshotStore::new(&snapshot_path)
        .expect("lsm_storage_engine snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement lsm_storage_engine snapshot"),
        vec![replacement_document.clone()]
    );
...
fn lsm_engine_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original LSM Engine".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to lsm_engine");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from lsm_engine after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement LSM Engine".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = LsmEngineSnapshotStore::new(&snapshot_path)
        .expect("lsm_engine snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = LsmEngineSnapshotStore::new(&snapshot_path)
        .expect("lsm_engine snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement lsm_engine snapshot"),
        vec![replacement_document.clone()]
    );
...
fn etchdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original EtchDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to etchdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from etchdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement EtchDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement etchdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn fastkv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original FastKV".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to fastkv");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from fastkv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement FastKV".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement fastkv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn apex_store_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original ApexStore".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to apex_store");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from apex_store after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement ApexStore".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = ApexStoreSnapshotStore::new(&snapshot_path)
        .expect("apex_store snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = ApexStoreSnapshotStore::new(&snapshot_path)
        .expect("apex_store snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement apex_store snapshot"),
        vec![replacement_document.clone()]
    );
...
fn lsmdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original LSMDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to lsmdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from lsmdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement LSMDB".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        LsmdbSnapshotStore::new(&snapshot_path).expect("lsmdb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement lsmdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn ferrumdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original FerrumDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to ferrumdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from ferrumdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect ferrumdb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement FerrumDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement ferrumdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn mmdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original MMDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to mmdb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from mmdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect mmdb deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement MMDB".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        MmdbSnapshotStore::new(&snapshot_path).expect("mmdb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement mmdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn mu_db_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original muDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to mu_db");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from mu_db after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement muDB".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        MuDbSnapshotStore::new(&snapshot_path).expect("mu_db snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement mu_db snapshot"),
        vec![replacement_document.clone()]
    );
...
fn mindb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let initial_document =
        backend::models::document::Document::new(document_id, Some("Initial Mindb".to_owned()));
    ...
    store
        ...
        .expect("initial snapshot should save to mindb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from mindb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect mindb deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Mindb".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        MindbSnapshotStore::new(&snapshot_path).expect("mindb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement mindb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn graus_db_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original GrausDb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to graus_db");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from graus_db after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect graus_db deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement GrausDb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement graus_db snapshot"),
        vec![replacement_document.clone()]
    );
...
fn yakv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original YAKV".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to yakv");
    drop(store);

    let reopened_store =
        YakvSnapshotStore::new(&snapshot_path).expect("yakv snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from yakv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect yakv deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement YAKV".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        YakvSnapshotStore::new(&snapshot_path).expect("yakv snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement yakv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn yakvdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original YAKVDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to yakvdb");
    drop(store);

    let reopened_store =
        YakvdbSnapshotStore::new(&snapshot_path).expect("yakvdb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from yakvdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect yakvdb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement YAKVDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement yakvdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn kv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original KV".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);

    let store = KvSnapshotStore::new(&snapshot_path).expect("kv snapshot store should initialize");
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to kv");
    drop(store);

    let reopened_store =
        KvSnapshotStore::new(&snapshot_path).expect("kv snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from kv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect kv deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement KV".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        KvSnapshotStore::new(&snapshot_path).expect("kv snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement kv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn eight_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Eight".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to eight");
    drop(store);

    let reopened_store =
        EightSnapshotStore::new(&snapshot_path).expect("eight snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from eight after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect eight deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement Eight".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        EightSnapshotStore::new(&snapshot_path).expect("eight snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement eight snapshot"),
        vec![replacement_document.clone()]
    );
...
fn epoch_db_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original EpochDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to epoch-db");
    drop(store);

    let reopened_store =
        EpochDbSnapshotStore::new(&snapshot_path).expect("epoch-db snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from epoch-db after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect epoch-db deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement EpochDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement epoch-db snapshot"),
        vec![replacement_document.clone()]
    );
...
fn rumdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original RumDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to rumdb");
    drop(store);

    let reopened_store =
        RumDbSnapshotStore::new(&snapshot_path).expect("rumdb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from rumdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect rumdb deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement RumDB".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        RumDbSnapshotStore::new(&snapshot_path).expect("rumdb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement rumdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn rustcask_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Rustcask".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to rustcask");
    drop(store);

    let reopened_store =
        RustcaskSnapshotStore::new(&snapshot_path).expect("rustcask snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from rustcask after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect rustcask deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Rustcask".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement rustcask snapshot"),
        vec![replacement_document.clone()]
    );
...
fn rusty_leveldb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original Rusty LevelDB".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to rusty-leveldb");
    drop(store);

    let reopened_store = RustyLeveldbSnapshotStore::new(&snapshot_path)
        .expect("rusty-leveldb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from rusty-leveldb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect rusty-leveldb deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Rusty LevelDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = RustyLeveldbSnapshotStore::new(&snapshot_path)
        .expect("rusty-leveldb snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = RustyLeveldbSnapshotStore::new(&snapshot_path)
        .expect("rusty-leveldb snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement rusty-leveldb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn hmdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original HmDb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to hmdb");
    drop(store);

    let reopened_store =
        HmdbSnapshotStore::new(&snapshot_dir).expect("hmdb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from hmdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect hmdb deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement HmDb".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        HmdbSnapshotStore::new(&snapshot_dir).expect("hmdb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement hmdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn saturn_snapshot_store_round_trips_document_catalog() {
    ...
    let snapshot_path = snapshot_dir.join("snapshots.saturn");
    let store =
        SaturnSnapshotStore::new(&snapshot_path).expect("saturn snapshot store should initialize");
    let document =
        backend::models::document::Document::new(Uuid::new_v4(), Some("Saturn".to_owned()));
    let snapshot = DocumentSnapshot::new(document.clone(), vec![1, 2, 3]);

    store
        .save_snapshot(snapshot)
        .expect("snapshot should save to saturn");

    let listed_documents = store
        .list_documents()
        .expect("document catalog should load from saturn");
    let loaded_snapshot = store
        .load_snapshot(&document.id)
        .expect("snapshot should load from saturn")
        .expect("snapshot should exist");
    ...
    let reopened =
        SaturnSnapshotStore::new(&snapshot_path).expect("saturn snapshot store should reopen");
    let reopened_documents = reopened
        .list_documents()
        .expect("document catalog should reload from saturn");
    let reopened_snapshot = reopened
        .load_snapshot(&document.id)
        .expect("snapshot should reload from saturn")
        .expect("snapshot should exist after reopen");
...
fn icefalldb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original IcefallDB".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to icefalldb");
    drop(store);

    let reopened_store = IcefalldbSnapshotStore::new(&snapshot_path)
        .expect("icefalldb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from icefalldb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement IcefallDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = IcefalldbSnapshotStore::new(&snapshot_path)
        .expect("icefalldb snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = IcefalldbSnapshotStore::new(&snapshot_path)
        .expect("icefalldb snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement icefalldb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn blazeup_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Blazeup".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to blazeup");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from blazeup after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Blazeup".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement blazeup snapshot"),
        vec![replacement_document.clone()]
    );
...
fn bitask_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original bitask".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to bitask");
    drop(store);

    let reopened_store =
        BitaskSnapshotStore::new(&snapshot_dir).expect("bitask snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from bitask after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect bitask deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement bitask".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement bitask snapshot"),
        vec![replacement_document.clone()]
    );
...
fn bitkv_rs_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original bitkv-rs".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to bitkv-rs");
    drop(store);

    let reopened_store =
        BitkvRsSnapshotStore::new(&snapshot_dir).expect("bitkv-rs snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from bitkv-rs after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect bitkv-rs deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement bitkv-rs".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement bitkv-rs snapshot"),
        vec![replacement_document.clone()]
    );
...
fn candystore_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original Candystore".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to candystore");
    drop(store);

    let reopened_store = CandystoreSnapshotStore::new(&snapshot_path)
        .expect("candystore snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from candystore after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect candystore deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Candystore".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = CandystoreSnapshotStore::new(&snapshot_path)
        .expect("candystore snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = CandystoreSnapshotStore::new(&snapshot_path)
        .expect("candystore snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement candystore snapshot"),
        vec![replacement_document.clone()]
    );
...
fn cuendillar_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original Cuendillar".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to cuendillar");
    drop(store);

    let reopened_store = CuendillarSnapshotStore::new(&snapshot_path)
        .expect("cuendillar snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from cuendillar after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect cuendillar deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Cuendillar".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = CuendillarSnapshotStore::new(&snapshot_path)
        .expect("cuendillar snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = CuendillarSnapshotStore::new(&snapshot_path)
        .expect("cuendillar snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement cuendillar snapshot"),
        vec![replacement_document.clone()]
    );
...
fn native_db_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original native_db".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to native_db");
    drop(store);

    let reopened_store =
        NativeDbSnapshotStore::new(&snapshot_path).expect("native_db snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from native_db after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect native_db deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement native_db".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement native_db snapshot"),
        vec![replacement_document.clone()]
    );
...
fn nebari_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Nebari".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to nebari");
    drop(store);

    let reopened_store =
        NebariSnapshotStore::new(&snapshot_root).expect("nebari snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from nebari after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect nebari deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Nebari".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement nebari snapshot"),
        vec![replacement_document.clone()]
    );
...
fn persistent_kv_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original PersistentKV".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to persistent_kv");
    drop(store);

    let reopened_store = PersistentKvSnapshotStore::new(&snapshot_path)
        .expect("persistent_kv snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from persistent_kv after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect persistent_kv deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement PersistentKV".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = PersistentKvSnapshotStore::new(&snapshot_path)
        .expect("persistent_kv snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = PersistentKvSnapshotStore::new(&snapshot_path)
        .expect("persistent_kv snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement persistent_kv snapshot"),
        vec![replacement_document.clone()]
    );
...
fn nodb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original NoDb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to nodb");
    drop(store);

    let reopened_store =
        NodbSnapshotStore::new(&snapshot_path).expect("nodb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from nodb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect nodb deletion")
            .is_empty()
    );
    drop(reopened_store);

    let replacement_document =
        backend::models::document::Document::new(document_id, Some("Replacement NoDb".to_owned()));
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
        NodbSnapshotStore::new(&snapshot_path).expect("nodb snapshot store should reopen");
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement nodb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn okofdb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original okofdb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to okofdb");
    drop(store);

    let reopened_store =
        OkofdbSnapshotStore::new(&snapshot_path).expect("okofdb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from okofdb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement okofdb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement okofdb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn celerix_store_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original Celerix".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to celerix_store");
    drop(store);

    let reopened_store = CelerixStoreSnapshotStore::new(&snapshot_path)
        .expect("celerix_store snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from celerix_store after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement Celerix".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = CelerixStoreSnapshotStore::new(&snapshot_path)
        .expect("celerix_store snapshot store should reopen");
    replacement_store
        .save_snapshot(replacement_snapshot)
        .expect("replacement snapshot should save after reopen");
    ...
    let final_store = CelerixStoreSnapshotStore::new(&snapshot_path)
        .expect("celerix_store snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement celerix_store snapshot"),
        vec![replacement_document.clone()]
    );
...
fn citadeldb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let original_document = backend::models::document::Document::new(
        document_id,
        Some("Original citadeldb".to_owned()),
    );
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to citadeldb");
    drop(store);
    ...
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from citadeldb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement citadeldb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store = CitadeldbSnapshotStore::new(
        &snapshot_path,
        b"test-citadel-snapshot-passphrase".as_slice(),
    )
    .expect("citadeldb snapshot store should reopen");
    ...
    let final_store = CitadeldbSnapshotStore::new(
        &snapshot_path,
        b"test-citadel-snapshot-passphrase".as_slice(),
    )
    .expect("citadeldb snapshot store should reopen");
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement citadeldb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn nikidb_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original NikiDb".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to nikidb");
    drop(store);

    let reopened_store =
        NikidbSnapshotStore::new(&snapshot_path).expect("nikidb snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from nikidb after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement NikiDb".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement nikidb snapshot"),
        vec![replacement_document.clone()]
    );
...
fn parity_db_snapshot_store_reuses_doc_id_after_delete_and_reopen() {
    ...
    let document_id = Uuid::new_v4();
    let original_document =
        backend::models::document::Document::new(document_id, Some("Original ParityDB".to_owned()));
    let original_snapshot = DocumentSnapshot::new(original_document.clone(), vec![1, 2, 3]);
    ...
    store
        .save_snapshot(original_snapshot)
        .expect("initial snapshot should save to parity_db");
    drop(store);

    let reopened_store =
        ParityDbSnapshotStore::new(&snapshot_path).expect("parity_db snapshot store should reopen");
    reopened_store
        .delete_snapshot(&document_id)
        .expect("snapshot should delete from parity_db after reopen");
    assert!(
        reopened_store
            .load_snapshot(&document_id)
            .expect("deleted snapshot lookup should succeed")
    ...
    assert!(
        reopened_store
            .list_documents()
            .expect("document catalog should reflect parity_db deletion")
            .is_empty()
    );
    drop(reopened_store);
    ...
    let replacement_document = backend::models::document::Document::new(
        document_id,
        Some("Replacement ParityDB".to_owned()),
    );
    let replacement_snapshot =
        DocumentSnapshot::new(replacement_document.clone(), vec![4, 5, 6, 7]);
    let replacement_store =
    ...
    assert_eq!(
        final_store
            .list_documents()
            .expect("document catalog should contain replacement parity_db snapshot"),
        vec![replacement_document.clone()]
    );
