...
- `SNAPSHOT_STORE`: `memory`, `file`, `agdb`, `flash_kv`, `blockbucket`, `grebedb`, `grumpydb`, `graus_db`, `highlandcows_isam`, `simple_db`, `docdb`, `eight`, `epoch_db`, `ferrumdb`, `rumdb`, `sqlite`, `heed`, `hightower_kv`, `hmdb`, `icefalldb`, `bitask`, `bitkv_rs`, `candystore`, `cuendillar`, `jammdb`, `mace`, `janql`, `fjall`, `persy`, `persistent_kv`, `native_db`, `nebari`, `nikidb`, `nodb`, `okofdb`, `parity_db`, `pickledb`, `rcask`, `microkv`, `redb`, `rskey`, `readb`, `rustlite`, `rustcask`, `rusty_leveldb`, `canopydb`, `caves`, `ckydb`, `crepedb`, `scdb`, `skv`, `surrealkv`, `sled`, `rustbreak`, `yedb`, `btree_store`, `siamesedb`, `structsy`, `abyssiniandb`, `aeternusdb`, `thunderdb`, `thetadb`, `tinybase`, `tinydb`, `dblite`, `dbless`, `db_rs`, `dharmadb`, `sanakirja`, `snaildb`, `tinykv`, `vsdb`, `yakv`, `saberdb`, `smolldb`, `kstone`, `roughdb`, `raindb`, `jsondb`, `kopperdb`, `kv`, `koit`, `lite_db`, `log_kv`, `lsm_storage_engine`, `lsmdb`, `mindb`, `mmdb`, `nanodb`, `jfs`, `json_store`, `feoxdb`, `s3`, 또는 `managed`
...
| 단일 노드 재시작 복구만 필요하고 파일 단위 백업/교체가 중요하나 | `file`, `agdb`, `jammdb`, `persy`, `native_db`, `nikidb`, `nodb`, `redb`, `crepedb`, `rskey`, `rustbreak`, `btree_store`, `structsy`, `abyssiniandb`, `surrealkv`, `thunderdb`, `thetadb`, `tinybase`, `tinydb`, `dblite`, `dbless`, `sanakirja`, `tinykv`, `vsdb`, `yakv`, `saberdb`, `smolldb`, `kstone`, `roughdb`, `raindb`, `jsondb`, `koit`, `lite_db`, `log_kv`, `lsm_storage_engine`, `lsmdb`, `mindb`, `mmdb`, `nanodb`, `jfs`, `json_store`, `simple_db`, `docdb`, `rcask` 중 단일 path 또는 단일 directory 기반 store를 우선 검토한다. `kopperdb`와 `grebedb`는 디렉터리 기반이지만 각각 단일 root 아래 세그먼트 로그 또는 단일 engine directory만 관리하고, `vsdb`는 `SNAPSHOT_VSDB_PATH/store.meta.json`과 process-global base dir 조합만 관리해 로컬 재기동 복구 절차가 단순한 편이다. |
| 디렉터리 단위 엔진 백업/restore 절차가 더 자연스러운가 | `flash_kv`, `heed`, `hightower_kv`, `hmdb`, `icefalldb`, `bitask`, `bitkv_rs`, `candystore`, `kopperdb`, `epoch_db`, `rumdb`, `fjall`, `parity_db`, `readb`, `kv`, `rustlite`, `rusty_leveldb`, `canopydb`, `ckydb`, `scdb`, `sled`, `yedb`, `siamesedb`, `snaildb`처럼 디렉터리 기반 store를 쓴다. |
...
| pure-Rust/no-bindgen/no-native-conflict 제약을 현재 빌드 그래프에서 유지해야 하나 | 현재 landed baseline은 `agdb`, `flash_kv`, `grebedb`, `grumpydb`, `graus_db`, `simple_db`, `docdb`, `eight`, `epoch_db`, `ferrumdb`, `rumdb`, `btree_store`, `siamesedb`, `structsy`, `abyssiniandb`, `aeternusdb`, `thunderdb`, `thetadb`, `tinybase`, `tinydb`, `dblite`, `dbless`, `db_rs`, `dharmadb`, `sanakirja`, `snaildb`, `tinykv`, `vsdb`, `yakv`, `saberdb`, `smolldb`, `kstone`, `roughdb`, `raindb`, `jsondb`, `kopperdb`, `kv`, `koit`, `lite_db`, `log_kv`, `lsm_storage_engine`, `lsmdb`, `mindb`, `mmdb`, `nanodb`, `jfs`, `json_store`, `persistent_kv`, `nikidb`, `nodb`, `readb`, `rustlite`, `rusty_leveldb`, `canopydb`, `caves`, `ckydb`, `crepedb`, `scdb`, `skv`, `surrealkv`, `rskey`, `hightower_kv`, `hmdb`, `icefalldb`, `bitask`, `bitkv_rs`, `candystore`, `nebari`, `rcask`다. `grebedb` upstream는 `rmp-serde` 0.15 계열에 묶여 있어 현재 workspace에서는 path-vendoring 패치가 필요했다. 추가 후보를 검토할 때도 native `links` 충돌과 bindgen 필요 여부를 먼저 배제한다. |
