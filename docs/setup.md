...
| 단일 노드 재시작 복구만 필요하고 파일 단위 백업/교체가 중요하나 | `file`, `jammdb`, `persy`, `native_db`, `redb`, `rustbreak`, `btree_store`, `structsy`, `abyssiniandb` 중 단일 path 기반 store를 우선 검토한다. |
...
| pure-Rust/no-bindgen/no-native-conflict 제약을 현재 빌드 그래프에서 유지해야 하나 | 현재 landed baseline은 `btree_store`, `siamesedb`, `structsy`, `abyssiniandb`다. 추가 후보를 검토할 때도 native `links` 충돌과 bindgen 필요 여부를 먼저 배제한다. |
...
- corrupt entry를 warning과 함께 건너뛰는 현재 catalog 정책을 적극 활용하려면 `jammdb`, `persy`, `native_db`, `redb`, `btree_store`, `siamesedb`, `abyssiniandb` 쪽이 기본값 후보로 더 안전하다.
