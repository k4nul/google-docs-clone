...
| 단일 노드 재시작 복구만 필요하고 파일 단위 백업/교체가 중요하나 | `file`, `agdb`, `jammdb`, `persy`, `native_db`, `nikidb`, `nodb`, `redb`, `crepedb`, `rskey`, `rustbreak`, `btree_store`, `structsy`, `abyssiniandb`, `surrealkv`, `thunderdb`, `thetadb`, `tinybase`, `tinydb`, `dblite`, `dbless`, `sanakirja`, `tinykv`, `vsdb`, `yakv`, `saberdb`, `smolldb`, `jsondb`, `koit`, `lite_db`, `lsm_storage_engine`, `mmdb`, `nanodb`, `jfs`, `json_store`, `simple_db`, `docdb`, `rcask` 중 단일 path 또는 단일 directory 기반 store를 우선 검토한다. `kopperdb`와 `grebedb`는 디렉터리 기반이지만 각각 단일 root 아래 세그먼트 로그 또는 단일 engine directory만 관리하고, `vsdb`는 `SNAPSHOT_VSDB_PATH/store.meta.json`과 process-global base dir 조합만 관리해 로컬 재기동 복구 절차가 단순한 편이다. |
