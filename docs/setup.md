...
| 디렉터리 단위 엔진 백업/restore 절차가 더 자연스러운가 | `flash_kv`, `heed`, `hightower_kv`, `hmdb`, `icefalldb`, `bitask`, `candystore`, `kopperdb`, `epoch_db`, `rumdb`, `fjall`, `parity_db`, `readb`, `kv`, `rustlite`, `rusty_leveldb`, `canopydb`, `ckydb`, `scdb`, `sled`, `yedb`, `siamesedb`, `snaildb`처럼 디렉터리 기반 store를 쓴다. |
