...
- 최신 백엔드 변경은 `RoomLocator`에 file-backed ownership resolver를 추가해 `local`, `static`, `file` 모드를 런타임에서 선택할 수 있게 정리한 것이다. `file` 모드는 `ROOM_COORDINATOR_STATE_DIR/<doc_id>.json`의 active owner state를 읽어 현재 `NODE_ID`와 다른 node가 기록된 문서에 대해 `409 conflict`와 `owner.node_id`를 반환한다.
...
- 현재 런타임은 여전히 단일 프로세스 owner 판정이 기본이며, `static` owner hints와 `file` owner state는 모두 best-effort 준비 경계 수준으로만 제공된다. 실제 멀티 프로세스 활성화는 지원하지 않는다.
...
- 다음 구현 후보는 위 계약을 실제 backend adapter와 heartbeat loop로 연결하는 것이다. 다만 authoritative coordination 저장소와 shared snapshot store가 아직 선택되지 않았으므로 현재 저장소 범위에서는 여전히 roadmap blocked 상태다.
