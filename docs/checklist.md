...
- sandboxed run에서는 commit/push/WebSocket 통합 테스트가 환경에 따라 차단될 수 있으므로, publish 전에는 `./scripts/preflight.sh publish`, socket 검증 전에는 `./scripts/preflight.sh websocket` 또는 unrestricted 실행으로 다시 확인한다.
