...
- 최신 백엔드 변경은 `RoomCoordinator`에 file-backed active room state를 추가해 `noop`, `logging`, `file` 모드를 런타임에서 선택할 수 있게 정리한 것이다. `file` 모드는 `ROOM_COORDINATOR_STATE_DIR/<doc_id>.json`에 현재 active owner 상태를 atomic write로 남기고 마지막 세션 종료 뒤 제거한다.
...
- 다음 구현 후보는 shared filesystem 또는 외부 저장소를 전제로 file-backed coordinator state를 다른 노드가 소비할 수 있는 ownership resolver/heartbeat 정책으로 확장하는 것이다. 다만 현재 저장소 범위에서는 여전히 roadmap blocked 상태다.
