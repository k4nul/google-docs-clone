...
- 최신 백엔드 변경은 `ROOM_COORDINATOR=sqlite` / `ROOM_LOCATOR=sqlite`를 추가해 `ROOM_COORDINATOR_SQLITE_PATH`의 shared SQLite lease row를 authoritative coordination backend로 사용할 수 있게 만든 것이다. `NODE_BASE_URL`이 있으면 sqlite lease row와 `409 conflict` 응답 모두 canonical `owner.base_url`을 포함한다.
...
- 다음 구현 후보는 sqlite lease backend를 실제 ingress redirect/proxy 레이어와 연결하고 owner handoff end-to-end 검증을 추가하는 것이다. shared SQLite DB 없이 동작하는 managed coordination backend는 여전히 roadmap blocked 상태다.
