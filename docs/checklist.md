...
- 최신 백엔드 변경은 `FileRoomCoordinator`/`FileRoomLocator`에 canonical lease record와 heartbeat를 도입한 것이다. `ROOM_COORDINATOR=file`은 `ROOM_COORDINATOR_STATE_DIR/<doc_id>.json`에 `doc_id`, `node_id`, optional `base_url`, `lease_id`, `epoch`, `acquired_at`, `renewed_at`, `expires_at`를 기록하고 background heartbeat로 lease를 갱신하며, `ROOM_LOCATOR=file`은 같은 state의 `expires_at`이 아직 지나지 않은 경우에만 non-local owner를 `409 conflict`와 `owner.node_id`로 반환한다.
...
- 현재 런타임은 여전히 단일 프로세스 owner 판정이 기본이며, `static` owner hints와 `file` lease state는 모두 best-effort 준비 경계 수준으로만 제공된다. 실제 멀티 프로세스 활성화는 지원하지 않는다.
...
- 다음 구현 후보는 shared snapshot store와 authoritative coordination backend를 선택해 현재 filesystem rehearsal 경계를 외부 adapter로 치환하는 것이다. file-backed heartbeat는 준비됐지만, authoritative CAS 보장과 shared snapshot durability가 없으므로 현재 저장소 범위에서는 여전히 roadmap blocked 상태다.
