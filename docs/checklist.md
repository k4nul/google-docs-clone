...
- 최신 백엔드 변경은 `SqliteSnapshotStore`를 추가해 `SNAPSHOT_STORE=sqlite`가 `SNAPSHOT_SQLITE_PATH` DB의 `snapshots` 테이블에 문서 metadata와 Yrs full-state update를 저장하고, startup hydrate/on-demand restore/list catalog를 같은 저장소에서 수행하게 만든 것이다.
...
- 현재 런타임은 여전히 단일 프로세스 owner 판정이 기본이며, `static` owner hints와 `file` lease state는 모두 best-effort 준비 경계 수준으로만 제공된다. `SNAPSHOT_STORE=sqlite`는 shared snapshot durability 후보를 제공하지만 실제 멀티 프로세스 활성화는 아직 지원하지 않는다.
