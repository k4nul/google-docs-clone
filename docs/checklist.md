...
- 최신 백엔드 변경은 `RoomCoordinator`를 `ROOM_COORDINATOR` 설정 경계로 승격해 `noop`와 `logging` 모드를 런타임에서 선택할 수 있게 정리한 것이다. logging 모드는 `NODE_ID`와 `doc_id` 기준 lifecycle trace만 남기고 lease/heartbeat side effect는 아직 붙이지 않는다.
