...
- 최신 백엔드 변경은 `NODE_BASE_URL`을 추가해 `ROOM_COORDINATOR=file`이 file-backed lease state에 canonical `base_url`을 기록하고, `ROOM_LOCATOR=file` conflict 응답도 optional `owner.base_url`을 반환하게 만든 것이다.
...
- 현재 런타임은 여전히 단일 프로세스 owner 판정이 기본이며, `static` owner hints와 `file` lease state는 모두 best-effort 준비 경계 수준으로만 제공된다. `NODE_BASE_URL`을 주면 file-backed lease도 redirect용 `owner.base_url`을 실어줄 수 있지만, `SNAPSHOT_STORE=sqlite` shared durability 후보만으로는 실제 멀티 프로세스 활성화를 지원하지 않는다.
...
- 다음 구현 후보는 authoritative coordination backend를 선택해 현재 file rehearsal locator/coordinator를 외부 adapter로 치환하는 것이다. shared snapshot durability 후보로 `SqliteSnapshotStore`는 준비됐지만, authoritative CAS 보장과 redirect/handoff owner contract가 아직 없어 현재 저장소 범위에서는 여전히 roadmap blocked 상태다.
