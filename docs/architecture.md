...
- `Config.snapshot_store`가 `memory`/`file`/`agdb`/`flash_kv`/`blockbucket`/`grebedb`/`highlandcows_isam`/`simple_db`/`docdb`/`eight`/`epoch_db`/`rumdb`/`shorterdb`/`sqlite`/`heed`/`hightower_kv`/`hmdb`/`icefalldb`/`bitask`/`bitkv_rs`/`candystore`/`jammdb`/`mace`/`janql`/`fjall`/`persy`/`persistent_kv`/`native_db`/`nebari`/`nikidb`/`nodb`/`parity_db`/`pickledb`/`rcask`/`microkv`/`redb`/`rskey`/`readb`/`rustlite`/`rustcask`/`rusty_leveldb`/`canopydb`/`caves`/`ckydb`/`scdb`/`skv`/`surrealkv`/`sled`/`rustbreak`/`yedb`/`btree_store`/`siamesedb`/`structsy`/`abyssiniandb`/`aeternusdb`/`thunderdb`/`thetadb`/`tinybase`/`dblite`/`dbless`/`db_rs`/`dharmadb`/`sanakirja`/`snaildb`/`tinykv`/`saberdb`/`smolldb`/`jsondb`/`kopperdb`/`kv`/`koit`/`lite_db`/`lsm_storage_engine`/`jfs`/`json_store`/`s3`/`managed` 어댑터 선택을 담당한다. `mace` 모드에서는 `SNAPSHOT_MACE_PATH` 디렉터리의 Mace `snapshots` bucket과 explicit `__catalog__` key가 snapshot storage 단위가 되고, `janql` 모드에서는 `SNAPSHOT_JANQL_PATH` 디렉터리의 `doc_id` key와 explicit `__catalog__` key가 snapshot storage 단위가 된다.
