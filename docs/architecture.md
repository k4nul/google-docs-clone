...
- 현재는 `InMemorySnapshotStore`, `FileSnapshotStore`, `SqliteSnapshotStore`, `JammdbSnapshotStore`, `FjallSnapshotStore`, `RedbSnapshotStore`, `SledSnapshotStore`, `S3SnapshotStore`, `ManagedSnapshotStore`가 연결되며, future adapter는 같은 trait으로 다른 db/object storage를 대체할 수 있다.
