...
- `src/storage`: snapshot store trait과 memory/file/sqlite/heed/jammdb/fjall/persy/native_db/redb/sled/s3/managed adapter
...
- 현재는 `InMemorySnapshotStore`, `FileSnapshotStore`, `SqliteSnapshotStore`, `HeedSnapshotStore`, `JammdbSnapshotStore`, `FjallSnapshotStore`, `PersySnapshotStore`, `NativeDbSnapshotStore`, `RedbSnapshotStore`, `SledSnapshotStore`, `S3SnapshotStore`, `ManagedSnapshotStore`가 연결되며, future adapter는 같은 trait으로 다른 db/object storage를 대체할 수 있다.
