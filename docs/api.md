...
- `assystem` 모드에서는 `SNAPSHOT_ASSYSTEM_PATH` 단일 파일의 `doc_id -> persisted snapshot JSON bytes` key-value entry를 scan해 catalog를 복구한다.
- `colon_db` 모드에서는 `SNAPSHOT_COLON_DB_PATH` 단일 파일의 `doc_id -> base64(persisted snapshot JSON)` row를 scan해 catalog를 복구한다.
- `append_kv` durability backend도 지원하며 `SNAPSHOT_APPEND_KV_PATH` 단일 append-only 파일의 explicit `__catalog__` key를 통해 catalog를 복구한다.
- `append_log` durability backend도 지원하며 `SNAPSHOT_APPEND_LOG_PATH` 단일 append-only 파일의 save/delete event replay로 catalog를 복구한다.
- `cacache` durability backend도 지원하며 `SNAPSHOT_CACACHE_PATH` content-addressed cache index의 `snapshot:<doc_id>` key listing으로 catalog를 복구한다.
- `png_db` durability backend도 지원하며 `SNAPSHOT_PNG_DB_PATH` 단일 PNG 파일의 compressed text row scan으로 catalog를 복구한다.
- `cdb64` durability backend도 지원하며 `SNAPSHOT_CDB64_PATH` 단일 CDB 파일의 key iteration으로 catalog를 복구한다.
...
- `data_pile` 모드에서는 `SNAPSHOT_DATA_PILE_PATH` append-only record 디렉터리에 save/delete 이벤트를 저장하고, startup replay 결과로 catalog를 복구한다.
...
- `kagi` 모드에서는 `SNAPSHOT_KAGI_PATH` 단일 whole-file bincode hashmap에 `doc_id -> persisted snapshot JSON string` entry를 저장하고, whole-file map iteration으로 catalog를 복구한다.
- `deeb` 모드에서는 `SNAPSHOT_DEEB_PATH` Deeb JSON database 파일의 `snapshots` entity에 `doc_id` primary key와 persisted snapshot JSON payload를 함께 저장하고, entity full scan으로 catalog를 복구한다.
- `mu_db` 모드에서는 `SNAPSHOT_MU_DB_PATH` data 파일과 같은 디렉터리의 `index_<file_name>` index 파일에 `snapshot:<doc_id>` payload key와 explicit `__catalog__` key를 저장하고, catalog key로 문서 목록을 복구한다.
...
- `fastkv` 모드에서는 `SNAPSHOT_FASTKV_PATH` 단일 compressed binary dump 파일에 `snapshot:<doc_id>` payload와 explicit `__catalog__` key를 유지하고, save/delete마다 temp dump rename으로 restart recovery 경계를 고정한다.
...
- `SNAPSHOT_STORE=etchdb`도 vendor-specific embedded database durability backend다.
...
- 필수 env는 `SNAPSHOT_DATA_PILE_PATH`다.
- data_pile catalog는 append-only record log의 save/delete 이벤트를 replay해 만든다. `GET /api/documents` catalog는 replay된 in-memory snapshot map에서 문서 메타데이터를 만든다.
...
- `SNAPSHOT_STORE=saturn`의 필수 env는 `SNAPSHOT_SATURN_PATH`다.
- saturn WAL store는 `snapshot:<doc_id> -> persisted snapshot JSON bytes` payload와 explicit `__catalog__` key를 저장하고, `GET /api/documents` catalog는 WAL replay 뒤 catalog key를 따라 각 payload를 복원해 문서 메타데이터를 만든다.
...
- 필수 env는 `SNAPSHOT_YAKVDB_PATH`다.
- yakvdb keyspace는 `snapshot:<doc_id> -> persisted snapshot JSON` key-value를 저장하고, `GET /api/documents` catalog는 min/above key traversal 뒤 각 payload를 복원해 문서 메타데이터를 만든다.
...
- 필수 env는 `SNAPSHOT_PNG_DB_PATH`다.
- png_db catalog는 단일 PNG 파일의 compressed text row chunk에 `doc_id`와 persisted snapshot JSON payload를 함께 저장하고, `GET /api/documents` catalog는 PNG row scan 뒤 각 payload를 복원해 문서 메타데이터를 만든다.
...
### `SNAPSHOT_STORE=append_log`

- 필수 env는 `SNAPSHOT_APPEND_LOG_PATH`다.
- append_log catalog는 append-only 단일 파일에 저장된 save/delete JSON event를 startup 때 replay해 만든다. `GET /api/documents` catalog는 replay된 in-memory snapshot map에서 문서 메타데이터를 만든다.

### `SNAPSHOT_STORE=data_pile`
...
- 필수 env는 `SNAPSHOT_KAGI_PATH`다.
- kagi catalog는 단일 bincode hashmap 파일의 `doc_id -> persisted snapshot JSON string` entry를 저장하고, `GET /api/documents` catalog는 whole-file map load 뒤 각 payload를 복원해 문서 메타데이터를 만든다.

### `SNAPSHOT_STORE=deeb`

- 필수 env는 `SNAPSHOT_DEEB_PATH`다.
- deeb catalog는 Deeb JSON database 파일의 `snapshots` entity에 `doc_id` primary key와 persisted snapshot JSON을 함께 저장하고, `GET /api/documents` catalog는 entity full scan 뒤 각 payload를 복원해 문서 메타데이터를 만든다.
...
### `SNAPSHOT_STORE=etchdb`

- 필수 env는 `SNAPSHOT_ETCHDB_PATH`다.
- etchdb catalog는 WAL-backed 디렉터리 store의 `snapshot:<doc_id> -> persisted snapshot JSON bytes` payload와 explicit `__catalog__` key를 저장하고, save/delete마다 `write_durable`로 WAL fsync 경계를 고정한다. `GET /api/documents` catalog는 startup WAL replay 뒤 catalog key를 읽고 각 payload를 다시 복원해 문서 메타데이터를 만든다.

### `SNAPSHOT_STORE=fastkv`

- 필수 env는 `SNAPSHOT_FASTKV_PATH`다.
- fastkv catalog는 compressed binary dump 파일의 `snapshot:<doc_id> -> persisted snapshot JSON bytes` payload와 explicit `__catalog__` key를 저장하고, save/delete마다 temp dump fsync 뒤 rename으로 파일을 확정한다. `GET /api/documents` catalog는 dump reload 뒤 catalog key를 읽고 각 payload를 다시 복원해 문서 메타데이터를 만든다.
...
- 필수 env는 `SNAPSHOT_JSON_DB_RS_PATH`다.
- json_db_rs catalog는 JSON 배열 event log 파일에 save/delete record를 append하고, `GET /api/documents` catalog는 whole-file event replay로 최신 payload를 복원해 문서 메타데이터를 만든다.
- 필수 env는 `SNAPSHOT_CDB64_PATH`다.
- cdb64 catalog는 단일 CDB 파일의 `doc_id -> persisted snapshot JSON bytes` key-value를 저장하고, save/delete마다 temp CDB rewrite 뒤 atomic rename으로 restart 복구 경계를 고정한다. `GET /api/documents` catalog는 whole-file key iteration 뒤 각 payload를 복원해 문서 메타데이터를 만든다.
- 필수 env는 `SNAPSHOT_JSON_MUTEX_DB_PATH`다.
- json_mutex_db catalog는 단일 JSON 파일 root object의 `doc_id -> persisted snapshot JSON` entry를 저장하고, save/delete마다 atomic save로 전체 파일을 교체한다. `GET /api/documents` catalog는 whole-file object load 뒤 각 payload를 복원해 문서 메타데이터를 만든다.
- 필수 env는 `SNAPSHOT_TOILETDB_PATH`다.
- toiletdb catalog는 단일 JSON 파일 root object의 `doc_id -> persisted snapshot JSON` entry를 저장하고, save/delete마다 ToiletDB temp file persist 뒤 file sync를 수행한다. `GET /api/documents` catalog는 whole-file object load 뒤 각 payload를 복원해 문서 메타데이터를 만든다.
- 필수 env는 `SNAPSHOT_DIR_CACHE_PATH`다.
- dir_cache catalog는 dir-cache 디렉터리의 `snapshot-<doc_id>.json` payload key와 explicit `__catalog__` key를 저장하고, save/delete마다 `sync()`를 호출한다. `GET /api/documents` catalog는 catalog key 뒤 각 payload를 다시 읽어 문서 메타데이터를 만든다.
- 필수 env는 `SNAPSHOT_MARBLE_PATH`다.
- marble catalog는 Marble object store의 fixed catalog object에 `doc_id -> object_id` mapping을 저장하고, snapshot payload는 별도 object로 저장한다. save/delete는 snapshot object와 catalog object를 같은 atomic write batch에 넣고 `fsync_each_batch=true`로 확정한다.
- 필수 env는 `SNAPSHOT_LMDB_RS_CORE_PATH`다.
- lmdb_rs_core catalog는 lmdb-rs-core environment의 main database에 `snapshot:<doc_id> -> persisted snapshot JSON` key와 explicit `__catalog__` key를 저장하고, save/delete commit 뒤 forced sync를 수행한다. `GET /api/documents` catalog는 catalog key 뒤 각 payload를 다시 읽어 문서 메타데이터를 만든다.
...
- 필수 env는 `SNAPSHOT_HURRAHDB_PATH`다.
- hurrahdb catalog는 single append-only AOF 파일의 `snapshot:<doc_id> -> persisted snapshot` key-value와 explicit `__catalog__` key를 저장하고, `GET /api/documents` catalog는 startup AOF replay 뒤 각 payload를 복원해 문서 메타데이터를 만든다.
- 필수 env는 `SNAPSHOT_FS_DB_PATH`다.
- fs-db catalog는 `SNAPSHOT_FS_DB_PATH/snapshot-<doc_id>.json` key-per-file JSON payload를 저장하고, `GET /api/documents` catalog는 디렉터리 file scan 뒤 각 payload를 복원해 문서 메타데이터를 만든다.
- 필수 env는 `SNAPSHOT_SQJSON_PATH`다.
- sqjson catalog는 `SNAPSHOT_SQJSON_PATH` 단일 JSON DB 파일에 `snapshot:<doc_id>:meta`와 base64 chunk key들을 저장하고, `GET /api/documents` catalog는 meta key scan 뒤 각 chunked payload를 복원해 문서 메타데이터를 만든다.
...
- 필수 env는 `SNAPSHOT_APEX_STORE_PATH`다.
- apex_store catalog는 ApexStore WAL/SSTable LSM engine에 `snapshot:<doc_id> -> persisted snapshot JSON bytes` payload와 explicit `__catalog__` key를 저장하고, `GET /api/documents` catalog는 WAL replay 뒤 catalog key를 따라 각 payload를 복원해 문서 메타데이터를 만든다.
- 필수 env는 `SNAPSHOT_GHALADB_PATH`다.
- ghaladb catalog는 GhalaDB LSM key/value store에 `snapshot:<doc_id> -> persisted snapshot JSON string` record와 explicit `__catalog__` key를 저장하고, `GET /api/documents` catalog는 catalog key 뒤 각 payload를 복원해 문서 메타데이터를 만든다. upstream bincode 2 API 호환성은 vendored patch로 고정한다.
