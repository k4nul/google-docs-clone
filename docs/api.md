...
- snapshot restore source는 현재 `SNAPSHOT_STORE=file|sqlite|heed|jammdb|fjall|persy|native_db|parity_db|pickledb|microkv|redb|sled|rustbreak|yedb|btree_store|siamesedb|structsy|s3|managed` 중 하나다.
...
- `SNAPSHOT_STORE=siamesedb`와 `SNAPSHOT_STORE=structsy`도 vendor-specific embedded database durability backend다.
