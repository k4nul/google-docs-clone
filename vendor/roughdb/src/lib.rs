...
impl StdError for Error {
    fn source(&self) -> Option<&(dyn StdError + 'static)> {
        match self {
            ...
            Self::NotFound | Self::Poisoned => None,
        }
    }
}
...
impl WriteBatch {
        ...
        V: AsRef<[u8]>,
    {
        self.operations.push(BatchOperation::Put(
            encode_bytes(key.as_ref()),
            encode_bytes(value.as_ref()),
        ));
    }
    ...
    {
        self.operations
            .push(BatchOperation::Delete(encode_bytes(key.as_ref())));
    }
}
...
impl Db {
    ...
    pub fn get(&self, key: &[u8]) -> Result<Vec<u8>, Error> {
        ...
        let Some(value) = state.entries.get(&encode_bytes(key)) else {
    ...
    pub fn flush(&self, _options: &FlushOptions) -> Result<(), Error> {
        let state = self.state.lock().map_err(|_| Error::Poisoned)?;
        persist_store(&self.path, &state)
    }
}
