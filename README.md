...
문서 단위의 실시간 협업 서버를 Rust로 안전하게 시작할 수 있도록 최소 실행 구조를 제공합니다. 현재 단계에서는 HTTP 헬스체크, 문서 생성/조회/삭제 API, 문서별 WebSocket 진입점, 관리용 API 토큰과 문서별 access token 기반 접근 제어, in-memory room registry, 그리고 memory/file/sqlite/jammdb/fjall/redb/sled/s3/managed snapshot 저장 추상화를 포함합니다.
...
- `SnapshotStore` trait 및 memory/file/sqlite/jammdb/fjall/redb/sled/s3/managed adapter
...
- `SNAPSHOT_STORE`: `memory`, `file`, `sqlite`, `jammdb`, `fjall`, `redb`, `sled`, `s3`, 또는 `managed`
...
- 테스트 가능한 앱 빌더 제공
- 기본 in-memory snapshot store와 로컬 file/sqlite/jammdb/fjall/redb/sled, S3-compatible object storage, external managed snapshot store 지원
