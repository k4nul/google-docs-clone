...
문서 단위의 실시간 협업 서버를 Rust로 안전하게 시작할 수 있도록 최소 실행 구조를 제공합니다. 현재 단계에서는 HTTP 헬스체크, 문서 조회 API, 문서별 WebSocket 진입점, in-memory room registry를 포함합니다.
...
- `GET /api/documents/:id` 문서 placeholder 조회 및 room 초기화
...
문서 상세 API를 먼저 호출하면 해당 `doc_id` room이 메모리에 준비되고, 이후 같은 `doc_id`로 WebSocket에 연결해 협업 세션을 시작할 수 있습니다. WebSocket 핸드셰이크의 `Origin` 헤더는 `FRONTEND_ORIGIN`과 정확히 일치해야 합니다.
