...
| pure-Rust/no-bindgen/no-native-conflict 제약을 현재 빌드 그래프에서 그대로 유지하기 | `agdb`, `grumpydb`, `btree_store`, `siamesedb`, `structsy`, `abyssiniandb`, `aeternusdb`, `thunderdb`, `thetadb`, `tinybase`, `tinydb`, `dblite`, `dbless`, `db_rs`, `dharmadb`, `sanakirja`, `snaildb`, `tinykv`, `yakv`, `saberdb`, `smolldb`, `kstone`, `roughdb`, `raindb`, `infusedb`, `kafi`, `tinkv`, `jsondb`, `kv`, `eight`, `epoch_db`, `ferrumdb`, `rumdb`, `koit`, `lite_db`, `log_kv`, `lsm_storage_engine`, `lsmdb`, `mindb`, `mmdb`, `nanodb`, `jfs`, `json_store`, `simple_db`, `docdb`, `shorterdb`, `highlandcows_isam`, `nikidb`, `nodb`, `persistent_kv`, `readb`, `rustlite`, `rustcask`, `rusty_leveldb`, `canopydb`, `caves`, `ckydb`, `crepedb`, `scdb`, `skv`, `surrealkv`, `rskey`, `hightower_kv`, `hmdb`, `icefalldb`, `bitask`, `bitkv_rs`, `bitcask_engine`, `candystore`, `celerix_store`, `nebari`, `rcask` | 최근 추가 backend가 이 제약을 실제 landed change로 통과했다. 같은 조건의 추가 후보를 검토할 때도 이 기준선을 먼저 보고, native `links` 충돌이나 bindgen 의존성이 생기면 제외한다. |
