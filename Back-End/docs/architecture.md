...
- `src/auth.rs`: Bearer token 파싱과 admin/document credential authorization helper
...
4. 문서 목록/생성은 configured `API_TOKEN`을 확인하고, 문서 상세/제목 변경/삭제는 문서별 `access_token`을 확인한다.
...
1. 클라이언트가 `GET /ws/:doc_id?access_token=<document-token>`로 업그레이드를 요청한다. non-browser 클라이언트는 `Authorization` 헤더도 사용할 수 있다.
...
4. 검증이 통과하면 `doc_id`에 해당하는 room을 조회하거나 snapshot store에서 on-demand로 복구한 뒤 문서 credential을 확인한다.
...
- 문서 메타데이터에는 외부 응답으로 노출하지 않는 `access_token`이 포함되며, 상세/제목 변경/삭제/WebSocket 진입 접근 제어에 사용된다.
...
- `SqliteRoomCoordinator`는 `ROOM_COORDINATOR_SQLITE_PATH`의 repository-local sqlite shim 파일 안 logical `room_leases` table에 canonical lease state를 upsert하고, active room 동안 background heartbeat로 `renewed_at`/`expires_at`을 갱신한 뒤 마지막 세션 종료 뒤 `node_id + lease_id + epoch` compare-and-delete로 정리하는 authoritative sqlite shim 구현이다. read/write는 같은 `<path>.lock` sidecar lock으로 serialize된다. `NODE_BASE_URL`이 있으면 canonical origin 형태의 `base_url`도 lease row에 포함한다.
...
- `ApexStoreSnapshotStore`는 `SNAPSHOT_APEX_STORE_PATH/store.json` repository-local apex_store shim map에 `snapshot:<doc_id>` payload와 explicit `__catalog__` key를 저장하고, startup hydrate/list 경로는 같은 catalog key를 그대로 사용한다.
...
- `NebariSnapshotStore`는 `SNAPSHOT_NEBARI_PATH/store.json` repository-local nebari shim의 `snapshots` named tree에 문서 metadata와 Yrs full-state update를 함께 저장하고, startup hydrate/list 경로는 same tree range scan을 그대로 사용한다. commit은 temp-write + rename 뒤 file/dir sync로 확정된다.
...
- `LoroKvSnapshotStore`는 `SNAPSHOT_LORO_KV_PATH` 단일 export 파일에 `doc_id -> persisted snapshot JSON bytes` payload를 저장하고, save/delete마다 repository-local `loro-kv-store` shim의 whole-store export를 temp+rename으로 확정하며 startup hydrate/list 경로는 full scan을 그대로 사용한다.
...
- `MindbSnapshotStore`는 `SNAPSHOT_MINDB_PATH` 디렉터리의 repository-local mindb shim에 `snapshot:<doc_id> -> persisted snapshot JSON` payload와 explicit `__catalog__` key를 함께 저장하고, current state는 `store.json`, replay log는 `wal.log`, manifest는 `manifest.json`에 유지한다. save/delete 뒤 `sync()`로 file-level durability 경계를 고정하고, recovery helper는 같은 WAL record를 재생해 startup hydrate/list 경로가 같은 catalog key를 그대로 사용하도록 고정한다.
- `MmdbSnapshotStore`는 `SNAPSHOT_MMDB_PATH/store.json` repository-local mmdb shim keyspace에 `snapshot:<doc_id> -> persisted snapshot JSON` payload와 explicit `__catalog__` key를 write batch로 함께 저장하고, sync write 뒤 flush해 startup hydrate/list 경로가 같은 catalog key를 그대로 사용하도록 고정한다.
...
- `KstoneSnapshotStore`는 `SNAPSHOT_KSTONE_PATH` 디렉터리의 repository-local kstone shim `wal.log` append-only keyspace에 `snapshot:<doc_id> -> persisted snapshot JSON bytes` payload와 explicit `__catalog__` key를 함께 기록하고, startup 때 same WAL replay 뒤 save/delete flush로 hydrate/list 경로가 같은 catalog key를 그대로 사용하도록 고정한다.
- `RoughdbSnapshotStore`는 `SNAPSHOT_ROUGHDB_PATH/store.json` repository-local roughdb shim keyspace에 `snapshot:<doc_id> -> persisted snapshot JSON bytes` payload와 explicit `__catalog__` key를 write batch로 함께 저장하고, sync write와 flush로 startup hydrate/list 경로가 같은 catalog key를 그대로 사용하도록 고정한다.
...
- 현재 저장소에는 side effect 없는 `NoopRoomCoordinator`, dry-run `LoggingRoomCoordinator`, local/shared filesystem에 lease state와 heartbeat를 남기는 `FileRoomCoordinator`, shared sqlite shim file에 lease state를 남기는 `SqliteRoomCoordinator`, 그리고 external lease service에 같은 lifecycle을 위임하는 `ManagedRoomCoordinator`가 들어가 있다.
...
- `SqliteRoomLocator`는 `ROOM_COORDINATOR_SQLITE_PATH`의 active owner lease row를 읽어 현재 `NODE_ID`와 다른 node가 기록돼 있고 `expires_at`이 아직 지나지 않았으면 non-local owner로 간주한다. 이 row는 repository-local sqlite shim 파일에 저장되며, lease row에 `base_url`이 있으면 conflict 응답에도 함께 전달해 redirect/proxy 결정을 돕는다.
...
- 현재는 `src/storage/mod.rs`의 `SUPPORTED_SNAPSHOT_STORES` canonical 목록에 대응하는 snapshot adapter, shared sqlite shim lease 기반 owner coordination, 그리고 external managed lease coordination이 있으므로 ownership coordination plane과 snapshot durability plane을 모두 shared sqlite shim 밖으로 분리할 수 있다. `ManagedRoomCoordinator`/`ManagedRoomLocator`를 `SqliteSnapshotStore`와 결합한 multi-host handoff rehearsal, `ManagedSnapshotStore` 자체의 저장/복구 경계, `S3SnapshotStore` startup/config 복구 경계, 그리고 managed coordination과 managed snapshot durability를 함께 묶은 handoff rehearsal까지 모두 회귀 테스트로 검증됐다.
...
- 현재 코드베이스의 `SqliteRoomCoordinator`/`SqliteRoomLocator`는 같은 계약을 shared sqlite shim file row에 매핑해 sidecar lock 기반 CAS를 제공한다.
