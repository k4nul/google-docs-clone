...
pub struct AbyssiniandbSnapshotStore {
    path: PathBuf,
}

impl AbyssiniandbSnapshotStore {
    pub fn new(path: impl Into<PathBuf>) -> Result<Self, StorageError> {
        let path = path.into();
        if path.as_os_str().is_empty() {
            return Err(StorageError::Config(
                "SNAPSHOT_ABYSSINIANDB_PATH cannot be empty when SNAPSHOT_STORE=abyssiniandb"
                    .to_owned(),
            ));
        }
        ...
        abyssiniandb::open_file(&path).map_err(|error| Self::map_database_error(&path, error))?;
...
impl SnapshotStore for AbyssiniandbSnapshotStore {
    fn load_snapshot(&self, doc_id: &Uuid) -> Result<Option<DocumentSnapshot>, StorageError> {
        let database = abyssiniandb::open_file(&self.path)
            .map_err(|error| Self::map_database_error(&self.path, error))?;
    ...
    fn save_snapshot(&self, snapshot: DocumentSnapshot) -> Result<(), StorageError> {
        ...
        let bytes = serde_json::to_vec(&PersistedSnapshot::from(snapshot)).map_err(|error| {
            StorageError::Io(format!(
                "failed to serialize abyssiniandb snapshot `{doc_id}`: {error}"
            ))
        })?;
        let mut snapshots = database
            .db_map_string(SNAPSHOTS_MAP)
            ...
            match snapshots.get(&doc_id_key) {
                Ok(Some(bytes)) => match self.deserialize_snapshot(doc_id, &bytes) {
                    ...
                    Err(StorageError::CorruptSnapshot(doc_id)) => tracing::warn!(
                        ...
                        "skipping corrupt abyssiniandb snapshot while building document catalog"
                    ),
                ...
                Ok(None) => tracing::warn!(
                    doc_id = %doc_id,
                    path = %self.path.display(),
                    "skipping missing abyssiniandb snapshot while building document catalog"
                ),
