...
mod tests {
    ...
    fn test_config() -> Config {
        let mut config = Config::from_env().expect("default test config should load");
        config.host = "127.0.0.1".to_owned();
        config.port = 4000;
        config.frontend_origin = "http://localhost:3000".to_owned();
        config.rust_log = "backend=debug".to_owned();
        config.api_token = "test-token".to_owned();
        config.snapshot_store = "memory".to_owned();
        config.snapshot_dir = "./data/snapshots".to_owned();
        config.room_locator = "local".to_owned();
        config.node_id = "local-node".to_owned();
        config.node_base_url = None;
        config.room_owner_hints_path = None;
        config
    }
    ...
    fn frontend_origin_header_rejects_origin_containing_newline() {
        let mut config = test_config();
        config.frontend_origin = "http://bad-origin\nX-Injected: evil".to_owned();
        let error = config
            .frontend_origin_header()
